                    ...
                    {sortedDirectors.map((person, idx) => (
                        ...
                        >
                            ...
                            <div className="relative h-full bg-[#FFFFF0] rounded-xl overflow-hidden border border-[#C9A24D] shadow-[0_4px_20px_rgba(59,45,95,0.1)] p-6 flex flex-col items-center text-center hover:border-[#D97706] transition-all duration-300 hover:-translate-y-1">
                                <div className={`relative mb-6 rounded-full overflow-hidden border-2 border-[#C9A24D]/30 group-hover:border-[#C9A24D] transition-all duration-300 shadow-md ${person?.featured ? 'w-48 h-48' : 'w-32 h-32'}`}>
                                    <Image
                                        src={person?.image || ''}
                                        alt={person?.name || ''}
                                        fill
                                ...
                                <p className="text-[#0F766E] font-medium tracking-wide uppercase text-sm mb-1">{person?.role}</p>
                                {person?.subRole && (
