      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-20 space-y-4">
          <motion.h2
            ...
            <span className="text-transparent bg-clip-text bg-linear-to-r from-purple-600 via-neutral-800 to-blue-600">
              The Visionaries
            </span>
          </motion.h2>
          <motion.p
          ...
          {sortedDirectors.map((person, idx) => (
            <motion.div
              ...
              <div className="relative h-full bg-white rounded-xl overflow-hidden border border-neutral-200 shadow-xl shadow-neutral-200/50 p-6 flex flex-col items-center text-center hover:border-purple-300 transition-all duration-300 hover:-translate-y-1">
                <div
                  className={`relative mb-6 rounded-full overflow-hidden border-2 border-neutral-100 group-hover:border-purple-400/50 transition-all duration-300 shadow-md ${person?.featured ? "w-48 h-48" : "w-32 h-32"}`}>
                  <Image
                    src={person?.image || ""}
                    alt={person?.name || ""}
                    fill
                ...
                <h3
                  className={`font-bold text-neutral-900 mb-2 ${person?.featured ? "text-2xl" : "text-xl"}`}>
                  {person?.name}
                </h3>

                <div className="h-0.5 w-12 bg-gradient-to-r from-purple-500 to-blue-500 mb-4 group-hover:w-24 transition-all duration-300" />

                <p className="text-purple-600 font-medium tracking-wide uppercase text-sm mb-1">
                  {person?.role}
                </p>
                {person?.subRole && (
            ...
            {managers.map((person, idx) => (
              <motion.div
                ...
                <div className="text-left">
                  <h4 className="text-neutral-900 font-bold text-xl group-hover:text-purple-600 transition-colors">
                    {person.name}
                  </h4>
